...
const Devices: React.FC = () => {
  ...
  const handleToggleStatus = async (device: Device) => {
    ...
    try {
      ...
      await deviceService.updateDeviceStatus(device.ID.toString(), { status: newStatus });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingDevice) {
        // Update existing device (status only for now)
        await deviceService.updateDeviceStatus(editingDevice.ID.toString(), { status: formData.status });
        setDevices(prev =>
          prev.map(d =>
            d.ID === editingDevice.ID ? { ...d, status: formData.status } : d
          )
        );
      } else {
        // Create new device
