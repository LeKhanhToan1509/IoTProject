    ...
    try {
      if (activeTab === 'login') {
        ...
        if (registrationStep === 1) {
          // Step 1: Request OTP
          try {
            const response = await authService.registerOTP({
              email: formData.email,
            });
            console.log('OTP sent successfully');
            setRegistrationStep(2);
            setVerificationSent(true);
            setError(response.message || 'OTP sent to your email. Please check your inbox.');
          } catch (error: any) {
          ...
          try {
            ...
            setError(response.message || 'Registration successful! Please sign in with your credentials.');
            setTimeout(() => setError(''), 3000);
          } catch (error: any) {
            console.error('Registration failed:', error);
            setError(error.response?.data?.message || 'Invalid OTP or registration failed. Please try again.');
          }
        }
      }
    } catch (error: any) {
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setLoading(true);
                      try {
                        const response = await authService.registerOTP({ email: formData.email });
                        setError(response.message || 'OTP code resent to your email.');
                        setTimeout(() => setError(''), 3000);
                      } catch (error: any) {
                        console.error('Resend OTP failed:', error);
                        setError(error.response?.data?.message || 'Failed to resend OTP code. Please try again.');
                      } finally {
                        setLoading(false);
                      }
                    }}
